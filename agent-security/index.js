...
const QUEUE_NAME  = 'agent-security-queue'
...
const SYSTEM_PROMPT = `You are an expert application security engineer specializing in OWASP Top 10 and secure coding practices.
Perform a thorough security audit of the provided code.
Identify injection risks, auth flaws, insecure dependencies, data exposure, hardcoded secrets, and all other vulnerabilities.

Respond ONLY with valid JSON matching this exact schema (no markdown, no extra text):
{
  "critical": ["array of strings — CVE-level vulnerabilities requiring immediate fix"],
  "high":     ["array of strings — serious vulnerabilities that should be fixed before deployment"],
  "medium":   ["array of strings — moderate risks that should be addressed"],
  "low":      ["array of strings — minor issues and hardening recommendations"]
}`
...
const worker = new Worker(QUEUE_NAME, async (job) => {
  ...
  const userContent =
    `Code to security-audit:\n\`\`\`\n${code}\n\`\`\`\n\nContext: ${description || 'Full security audit'}`
  ...
  try {
    ...
    output = { critical: [rawText], high: [], medium: [], low: [] }
  }
