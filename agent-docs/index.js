...
const AGENT_NAME  = 'docs'
const QUEUE_NAME  = 'agent-docs-queue'
...
const SYSTEM_PROMPT = `You are a technical writer and senior software engineer.
Write complete, professional documentation for the provided code including JSDoc comments, a README section, and a concise summary.

Respond ONLY with valid JSON matching this exact schema (no markdown wrapper, no extra text):
{
  "jsdoc":   "string — JSDoc / docstring comments for all functions and classes in the code",
  "readme":  "string — a README.md section describing what the code does, its API, parameters, return values, and usage examples",
  "summary": "string — a 2-4 sentence plain-English summary of what the code does and its purpose"
}`
...
const worker = new Worker(QUEUE_NAME, async (job) => {
  ...
  const userContent =
    `Code to document:\n\`\`\`\n${code}\n\`\`\`\n\nContext: ${description || 'Write complete documentation'}`
  ...
  try {
    ...
    output = { jsdoc: rawText, readme: '', summary: '' }
  }
